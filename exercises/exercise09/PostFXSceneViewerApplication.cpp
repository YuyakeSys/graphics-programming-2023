...
void PostFXSceneViewerApplication::InitializeFramebuffers()
{
    ...
    // Create temporary textures and framebuffers
    for (int i = 0; i < 2; i++) {
        m_2tempTextures[i] = std::make_shared<Texture2DObject>();
        m_2tempTextures[i]->Bind();
        m_2tempTextures[i]->SetImage(0, width, height, TextureObject::FormatRGBA, TextureObject::InternalFormat::InternalFormatRGBA16F);
        m_2tempTextures[i]->SetParameter(TextureObject::ParameterEnum::WrapS, GL_CLAMP_TO_EDGE);
        m_2tempTextures[i]->SetParameter(TextureObject::ParameterEnum::WrapT, GL_CLAMP_TO_EDGE);
        m_2tempTextures[i]->SetParameter(TextureObject::ParameterEnum::MinFilter, GL_LINEAR);
        m_2tempTextures[i]->SetParameter(TextureObject::ParameterEnum::MagFilter, GL_LINEAR);

        m_2tempFramebuffer[i] = std::make_shared<FramebufferObject>();
        m_2tempFramebuffer[i]->Bind();
        m_2tempFramebuffer[i]->SetTexture(FramebufferObject::Target::Draw, FramebufferObject::Attachment::Color0, *m_2tempTextures[i]);
        m_2tempFramebuffer[i]->SetDrawBuffers(std::array<FramebufferObject::Attachment, 1>({ FramebufferObject::Attachment::Color0 }));
    }
    Texture2DObject::Unbind();
    FramebufferObject::Unbind();
}
...
void PostFXSceneViewerApplication::InitializeRenderer()
{
    ...
    m_renderer.AddRenderPass(std::make_unique<PostFXRenderPass>(copyMaterial, m_2tempFramebuffer[0]));
    ...
    m_renderer.AddRenderPass(std::make_unique<PostFXRenderPass>(m_bloomMaterial, m_2tempFramebuffer[0]));

    // (todo) 09.3: Add blur passes
    std::shared_ptr<Material> blurHorizontalMaterial = CreatePostFXMaterial("shaders/postfx/blur.frag", m_2tempTextures[0]);
    blurHorizontalMaterial->SetUniformValue("Scale", glm::vec2(1.0f / width, 0.0f));
    std::shared_ptr<Material> blurVerticalMaterial = CreatePostFXMaterial("shaders/postfx/blur.frag", m_2tempTextures[1]);
    blurVerticalMaterial->SetUniformValue("Scale", glm::vec2(0.0f, 1.0f / height));
    for (int i = 0; i < 1; ++i)
    {
        m_renderer.AddRenderPass(std::make_unique<PostFXRenderPass>(blurHorizontalMaterial, m_2tempFramebuffer[1]));
        m_renderer.AddRenderPass(std::make_unique<PostFXRenderPass>(blurVerticalMaterial, m_2tempFramebuffer[0]));
    }
    ...
    m_composeMaterial->SetUniformValue("BloomTexture", m_2tempTextures[0]);
